...
(function ($) {
    ...
    $.fn.radioTool = function () {
        ...
        if (codeName != undefined && codeName != "") {
            R = radioTool.getDataByCode(zProps.codeName);
        }
        //如果是从后台获取数据
        if (url != undefined && url != "") {
            R = radioTool.getDataByUrl(zProps.url);
        }
        //如果是从枚举获取数据
        if (enumName != undefined && url != "") {
            R = radioTool.getDataByEnum(zProps.enumName);
        }
        radioTool.renderData(R, $grid, zProps);
    };
    /*默认配置*/
    var zProps = {};
    /*方法对象*/
    var radioTool = {
        ...
        /**单选组件数据渲染 by zhangyantao 2017/7/20*/
