...
(function ($) {
    ...
    $.fn.Hupload = function () {
        ...
        //将参数转为json
        zProps = eval("({" + zProps + "})");
        Hupload.renderData($div, zProps);
    };
    /*默认配置*/
    var zProps = {};
    /*方法对象*/
    var Hupload = {
        /**zhangyantao 2017/11/18 上传**/
        uploadInit: function (zProps) {
            //上传控件id
            var uploadId = zProps.uploadId;
            //上传地址
            var url = zProps.url || "/getData/uploads/";
            //上传按钮名称
            var btnName = zProps.btnName || '上传文件';
            //是否显示删除按钮
            var deleteBtn = zProps.deleteBtn || 'true';
        ...
        /**zhangyantao 2017/11/18 文件回填**/
        showFile: function (zProps) {
            //是否显示删除按钮
            var deleteBtn = zProps.deleteBtn || 'true';
            //上传控件id
            var uploadId = zProps.uploadId;
        ...
        /**组件渲染 by zhangyantao 2017/7/20*/
        renderData: function ($div, zProps) {

            var $div = $div;
            //上传控件id
            zProps.uploadId=zProps.uploadId?zProps.uploadId:$t.getUUID();
            var _uploadId = zProps.uploadId;
            if(!_uploadId){return false;}
            //是否显示上传按钮
            var uploadBtn = zProps.uploadBtn || 'true';
            //是否需要表单提交
            ...
            if(uploadBtn=="true"){
                //上传按钮渲染
                Hupload.uploadInit(zProps);
            }
            //文件回填
            Hupload.showFile(zProps);
        }
    }

})(jQuery);
/**zhangyantao 2017/11/18 删除**/
...
$(document).ready(function () {
    var Huploads = $("[zType='HuploadTool']");
