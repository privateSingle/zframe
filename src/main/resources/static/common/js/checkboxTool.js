...
(function ($) {
    ...
    $.fn.checkboxTool = function () {
        ...
        if (codeName != undefined && codeName != "") {
            R = checkboxTool.getDataByCode(zProps.codeName);
        }
        //如果是从后台获取数据
        if (url != undefined && url != "") {
            R = checkboxTool.getDataByUrl(zProps.url);
        }
        //如果是从枚举获取数据
        if (enumName != undefined && url != "") {
            R = checkboxTool.getDataByEnum(zProps.enumName);
        }
        checkboxTool.renderData(R, $grid, zProps);
    };
    /*默认配置*/
    var zProps = {};
    /*方法对象*/
    var checkboxTool = {
        /**通过url获取数据 by zhangyantao 2017/7/5*/
        ...
        renderData: function (R, $grid, zProps) {
            ...
            //是否打开全选按钮
            var allBtn = zProps.allBtn||'false';
            if(allBtn=='true'){
...
$(document).ready(function () {
    //下拉树查询
    var checkboxs = $("[zType='checkboxTool']");
