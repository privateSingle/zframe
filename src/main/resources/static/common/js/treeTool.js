...
(function ($) {
    ...
    $.fn.treeTool = function () {
        ...
        $(this).after('<input value="' + _value + '"  style="display: none" id="' + _id + '_id"  name="' + zProps.name + '"  class="layui-input">' +
...
function openZtree(obj) {
    ...
    $(obj).after('<input  id="' + _id + '_id"  style="display: none" name="' + zProps.name + '" value="' + _value + '" class="layui-input">' +
        '<div id="treeLayer" style="display: none;padding:10px;"> ' +
        '<ul id="zTree" class="ztree"></ul> ' +
        '</div>');
    $.ajax({
...
$(document).ready(function () {
    $("[zType='treeTool']").click(function () {
        var obj = $(this);
        openZtree(obj);
    });
    //下拉树查询
    var treeTools = $("[zType='treeTool']");
