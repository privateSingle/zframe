...
(function ($) {
    ...
    $.fn.linkSelectTool = function () {
        ...
        zProps = eval("({" + zProps + "})");
        //获取顶级id
        var topId=zProps.topId;
        zProps.normalUrl= zProps.url+"/"+topId;

        var R = selectTool.getDataByUrl(zProps.normalUrl);
        ...
        renderData: function (R, $grid, zProps) {
            ...
            if(_value!=""&&_values != undefined){
                ...
                for(var j=0;j<_values.length;j++){
                    if(_values[j]!=""){
                        ...
                        //获取所有下级数据
                        var LowerData = selectTool.getDataByUrl(zProps.url+"/"+ _values[j]);
                        var curr_data=LowerData.data;
                        if(curr_data){
                            var _div = '<div id="div_' + _values[j] + '" class="layui-input-inline" ></div>';
                            $(_grid).parent().append(_div);
                            var _select = '<select id="select_' + _values[j] + '" name="'+zProps.name+'" lay-search lay-filter="link" ></select>';
            ...
            layui.use(['form'], function () {
                ...
                form.on('select(link)', function (data) {
                    ...
                    if (parentId != "") {
                        //获取所有下级地区
                        var R = selectTool.getDataByUrl(zProps.url+"/"+ parentId);

                        if (R.data&&R.data.length > 0) {
                            var _div = '<div id="div_' + data.value + '" class="layui-input-inline" ></div>';
                            $(_grid).after(_div);
                            var _select = '<select id="select_' + data.value + '" name="'+zProps.name+'" lay-search lay-filter="link" ></select>';
...
$(document).ready(function () {
    //下拉树查询
    var linkSelects = $("[zType='linkSelectTool']");
