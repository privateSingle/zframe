...
$(function () {
    ...
    $(".sidebar .sidebar-menu").on("click", "a.cy-page", function () {
        ...
        if ($("#navTab").find("li[data-url='" + url + "']").length === 0) {
            ...
            $(".content").find("iframe").removeClass("z-show");
            //打开iframe
            var iframe = $('<iframe class="z-show" scrolling="yes" frameborder="0" style="width: 100%; height: 100%; overflow: visible; "></iframe>');
            $(iframe).attr("src", url);
            ...
            $(".content").find("iframe").removeClass("z-show");
            $(".content").find("iframe[src='"+url+"']").addClass("z-show");
            //更多列表
            $(".tabsMoreList").find("li").removeClass("selected");
