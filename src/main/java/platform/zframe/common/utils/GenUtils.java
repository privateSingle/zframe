...
import platform.zframe.entity.Column;
import platform.zframe.entity.Table;
...
public class GenUtils {
	...
	public static void generatorCode(Map<String, String> table,
		...
		map.put("classname", tableEntity.getClassname());
		map.put("pathName", Underline2Camel.camel2Bias(tableEntity.getClassname()));
		map.put("ftlPath",  "app/"+config.getString("package").replace(".", "/") +"/"+StringUtil.toLowerCaseFirstChar(className));
		map.put("columns", tableEntity.getColumns());
		map.put("package", "platform.app."+config.getString("package")+"."+StringUtil.toLowerCaseFirstChar(className));
		map.put("dirPath", "platform.app/"+config.getString("package")+"/"+StringUtil.toLowerCaseFirstChar(className));
		...
		String dirPath=config.getString("package").replace(".", "/") +"/"+StringUtil.toLowerCaseFirstChar(className);
		for(String template : templates){
			...
			try {
				//添加到zip
				zip.putNextEntry(new ZipEntry(getFileName(template, tableEntity.getClassName(), config.getString("package"),dirPath)));
	 ...
	 * 获取文件名
	 */
	public static String getFileName(String template, String className, String packageName,String dirPath){
		String packagePath = "main" + File.separator + "java" + File.separator;
		if(StringUtils.isNotBlank(packageName)){
			packagePath += packageName.replace(".", File.separator) + File.separator+StringUtil.toLowerCaseFirstChar(className)+ File.separator;
		}
		...
		if(template.contains("Mapper.xml.vm")){
			return "main" + File.separator + "resources" +  File.separator + "mapper"+File.separator+dirPath
		...
		if(template.contains("list.ftl.vm")){
			return "main" + File.separator + "resources" +  File.separator + "templates"+File.separator+dirPath
					+ File.separator + "list.ftl";
		}
		if(template.contains("add.ftl.vm")){
			return "main" + File.separator + "resources" +  File.separator + "templates"+File.separator+dirPath
					+ File.separator + "add.ftl";
		}
		if(template.contains("edit.ftl.vm")){
			return "main" + File.separator + "resources" +  File.separator + "templates"+File.separator+dirPath
					+ File.separator  + "edit.ftl";
		}
		if(template.contains("info.ftl.vm")){
			return "main" + File.separator + "resources" +  File.separator + "templates"+File.separator+dirPath
					+ File.separator  + "info.ftl";
		}
		if(template.contains("list.js.vm")){
			return "main" + File.separator + "resources" +  File.separator + "templates"+File.separator+dirPath
					+ File.separator  + "js"+ File.separator + "list.js";
		}
		if(template.contains("common.ftl.vm")){
			return "main" + File.separator + "resources" +  File.separator + "templates"+File.separator+dirPath
					+ File.separator + "common.ftl";
		}
